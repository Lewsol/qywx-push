...
所有路径都必须先安排维护窗口、停止全部旧实例、完整备份数据库及 sidecar 文件，并生成和安全备份新的 32 字符密钥。不要把旧、新密钥写在命令参数中；通过临时环境变量或部署平台 Secret 注入。

**路径 A：旧密钥已经满足当前 32 字节格式，且数据库已由当前安全版本管理。**

1. 使用旧密钥在隔离维护环境成功启动当前版本，让 callback Token 迁移、安全清理和通知 Token 一次性轮换全部完成；不要恢复公网流量。
2. 停止应用，确认 `callback_token_secure_purge_v1` 标记为 `complete`。
3. 按下方流程运行 `npm run rotate-key`。
4. 使用新密钥启动，重新查询并分发新的通知 URL，完成验证后再恢复流量。

**路径 B：历史密钥依赖旧版补齐/截断语义，或仍是没有 callback Token 加密字段的原始 schema。**

当前运行时会拒绝短于、长于 32 字节或非 ASCII 的原始旧密钥，因此不要尝试先用该原始值启动当前版本，也不要直接把环境变量替换成新密钥。保持服务停止，直接把原始旧值作为 `OLD_ENCRYPTION_KEY` 运行轮换脚本；脚本仅在此路径复现旧版补齐/截断规则，迁移 CBC 字段。成功后用新密钥首次启动当前版本，服务会再事务迁移明文 callback Token、完成安全清理并为所有历史配置生成新的通知 Token。验证和更新全部调用方之前不得恢复公网流量。

如果数据库已经存在新的 callback Token 加密字段，但安全清理标记不完整，同时原始旧密钥又不符合当前格式，说明升级处于不受支持的部分迁移状态；不要继续尝试换钥，应恢复升级前完整备份并重新按路径 B 执行。
